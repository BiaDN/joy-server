...
export default class GameService {
  ...
  async createCharacter(name: string, avatar: string, user: any): Promise<any> {
    ...
    return { data: character.toJSON() };
  }

  async equipCharacter(idCharacter: any, idUser: string): Promise<any> {
    const character = await Users.findOneAndUpdate(
      { _id: idUser },
      { characterEquipment: idCharacter },
      { new: true }
    );
    return { data: character?.toJSON() };
  }
  ...
  async equipItemCharacter(idCharacter: string, idItem: string): Promise<any> {
    const character = await Characters.findOneAndUpdate(
      { _id: idCharacter },
      { itemEquipment: idItem },
      { new: true }
    );
    return { data: character?.toJSON() };
  }
